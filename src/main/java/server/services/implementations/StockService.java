...
public class StockService implements IStockService {
	...
	public Stock findByCode(String code) {
		...
		var response = repository.findById(code).orElse(null);
	...
	public Stock createStock(String storeCode, String productCode, int quantity) {
		try {
			...
			log.info("[StockService][createStock]: Stock created for store {} and product {} with quantity {}",
					storeCode, productCode, quantity);
	...
	public boolean stockExists(String productCode, String storeCode) {
		...
		var exists = repository.findByProduct_CodeAndStore_Code(productCode, storeCode);
	...
	public Stock addStock(String code, int quantityToAdd) {
		try {
			...
			log.info("[StockService][addStock]: Successfully added {} units to stock with code {}", quantityToAdd,
					code);
			return updatedStock;
	...
	public Stock subtractStock(String code, int quantityToSubtract) {
		try {
			...
			Stock updatedStock = repository.save(stock);
			log.info("[StockService][subtractStock]: Successfully subtracted {} units from stock with code {}",
					quantityToSubtract, code);
			return updatedStock;
		} catch (NoSuchElementException e) {
